...
export default function PostGrid({ post }) {
  const { content, id, Images, Likes, Comments } = post;
  ...
  if (Images[0]) {
    return (
      <div className="h-full w-full aspect-square group cursor-pointer transition duration-300 hover:scale-110">
        <Image className={"w-full h-full"} cloudName={VITE_CLOUD_NAME} publicId={Images[0].url} alt="noimg">
          <Transformation aspectRatio="1:1" crop="fill"/>
        </Image>
        {userLogged && userLogged[0] && <HoverInfo Likes={Likes} Comments={Comments} post_id={id} />}
      </div>
    );
