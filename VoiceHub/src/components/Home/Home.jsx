...
export default function Home() {
  ...
  return (
    <div className="flex flex-col h-full justify-start bg-gradient-to-b from-sixty-percent-home to-sixty-percent min-h-screen">
      <Header />
      {modal ? <div className="fixed inset-0 transition-opacity z-40 flex justify-center items-center">
        <div className="absolute inset-0 bg-gray-800 opacity-75 z-40" onClick={() => setModal(false)}></div>
        <Upload setPublished={setPublished} published={published} />
      </div> : <></>}
      <div className="p-2">
        <button className="bg-ten-percent text-white p-2 rounded-md" w="3rem" onClick={() => setModal(!modal)}>
          Create
        </button>
      </div>
